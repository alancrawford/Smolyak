...
@ngenerate N (Array{Float64,2}) function Make_Grid(NGP::Int64,inds::Array{Array{Int64,1},1},mu::NTuple{N,Int64}...)
	...
	zH = 0
	for i = 1:length(inds)
		for j in product((@ntuple N k->gridtheta_A_i(inds[i][k]))...)
			zH += 1
			H[zH,:] = [j...]
		end
	end
...
function theta2x(thetapts::Array{Float64,2},lb::Vector{Float64},ub::Vector{Float64})
	centers = lb + (ub - lb)./2
    radii = (ub - lb)./2
    xpts = centers' .+ cos(thetapts).*radii'::Array{Float64,2}
	return xpts
end

function x2theta(xpts::Array{Float64,2},lb::Vector{Float64},ub::Vector{Float64})
 	centers = lb + (ub - lb)./2
    radii = (ub - lb)./2
    thetapts = acos((xpts .- centers')./radii')::Array{Float64,2}
	return thetapts
end
...
type SmolyakGrid
	...
	thetaGrid 	::	Array{Float64,2}	# Smolyak Grid on theta = [0,π]
	...
	function SmolyakGrid(	D::Int64, mu::IntOrVec,
		...
		thetaGrid = Make_Grid(NumGrdPts,Ginds,mubar...)	
		xGrid  = theta2x(thetaGrid,lb,ub)
		Binds = BasisIdx(NumGrdPts,Ginds,maxmu,mubar...)

		new(D, mu, NumGrdPts, lb, ub, thetaGrid, xGrid, Binds)
	end
end
