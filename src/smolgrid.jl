...
function SmolyakGrid(sk::SmolyakKernel) where T<:SmolyakKernelType
	...
	NGP = NumGridPts(sk.GridIdx,(sk.mu...,)) 
	grid = Vector{Float64}[Array{Float64}(undef,sk.D) for r in 1:NGP]
	grid!(grid,sk.GridIdx,tuple(sk.mu...,))			# Make Grid on [-1,1]
...
@generated function grid!(H::VV{Float64},inds::VV{Int64},mu::NTuple{N,T}) where N where T<:Real
	quote 
		zH = 0
		for i = 1:length(inds)
...
# Get grid
grid(sg::SmolyakGrid) = deepcopy(sg.grid)
xgrid(sg::SmolyakGrid) = z2x(sg.grid, sg.sk)
